...
const StyledPanel = styled.div<{ isPushed: boolean; showMenu: boolean, isMobile: boolean }>`
  position: fixed;
  padding-top: ${({ isMobile }) => (isMobile ? "75px" : 0)};
  margin-top: ${({ isMobile, showMenu }) => (!isMobile && showMenu ? "75px" : 0)};
  top: 0;
  left: 0;
  display: flex;
  ...
  height: calc(100vh - ${({ isMobile, showMenu }) => (isMobile || !showMenu ? 0 : 80)}px);
